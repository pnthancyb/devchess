...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // AI Move endpoint - Simplified and robust
  app.post('/api/chess/ai-move', async (req, res) => {
    try {
      const { fen, difficulty = 5, model = 'stockfish' } = req.body;
      ...
      // Use a strategic move selection based on difficulty
      let selectedMove;
      if (difficulty <= 3) {
        // Easy: Random move
        selectedMove = possibleMoves[Math.floor(Math.random() * possibleMoves.length)];
      } else if (difficulty <= 6) {
        // Medium: Slightly better moves (avoid obvious blunders)
        const nonBlunderMoves = possibleMoves.filter(move => {
          const testChess = new Chess(fen);
          testChess.move(move);
          return !testChess.isCheck(); // Avoid moves that put own king in check
        });
        const movesToConsider = nonBlunderMoves.length > 0 ? nonBlunderMoves : possibleMoves;
        ...
        const tacticalMoves = [...captures, ...checks];
        
        if (tacticalMoves.length > 0 && Math.random() < 0.7) {
      ...
      res.json({
        ...
        analysis: {
          score: 0,
          evaluation: `AI move at difficulty ${difficulty}`
        }
      });
